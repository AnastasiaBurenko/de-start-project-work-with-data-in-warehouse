    ...
    SELECT  
            ...
            FROM (
                SELECT     -- в этой выборке объединяем две внутренние выборки по расчёту столбцов витрины и применяем оконную функцию для определения самой популярной категории товаров
                        *,
                        RANK() OVER(PARTITION BY T2.customer_id ORDER BY count_product_for_type DESC) AS rank_count_product,
                        RANK() OVER(PARTITION BY T2.customer_id ORDER BY count_product_for_top_craftsman DESC) AS rank_count_craftsman
                        FROM ( 
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбца с самой популярной категорией товаров у мастера. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним JOIN
                                ...
                                INNER JOIN (
                                    SELECT     -- Эта выборка поможет определить самый популярный товар у заказчика. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка
                                            ...
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.product_type
                                                    ORDER BY count_product_for_type DESC
                                            ) AS T3 ON T2.customer_id = T3.customer_id_for_product_type
								INNER JOIN(
									SELECT
										dd.customer_id AS customer_id_for_top_craftsman,
										dd.craftsman_id,
										COUNT(dd.product_id) AS count_product_for_top_craftsman
										FROM dwh_delta AS dd
											GROUP BY dd.customer_id, craftsman_id
												ORDER BY count_product_for_top_craftsman DESC
											) AS T5 ON T5.customer_id_for_top_craftsman = T2.customer_id
                ) AS T4 WHERE T4.rank_count_product = 1 AND T4.rank_count_craftsman = 1 -- условие помогает оставить в выборке первую по популярности категорию товаров и мастеера
...
dwh_delta_update_result AS ( -- делаем перерасчёт для существующих записей витрины, так как данные обновились за отчётные периоды. Логика похожа на insert, но нужно достать конкретные данные из DWH
    SELECT 
            ...
            FROM (
                ...
                ) AS T4 WHERE T4.rank_count_product = 1  AND T4.rank_count_craftsman = 1
                ORDER BY report_period
),
